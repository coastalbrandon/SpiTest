...
typedef struct
{
    ...
    le_spi_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "le_spi_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_spi_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_spi_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_spi_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for le_spi disconnected\n");
}
